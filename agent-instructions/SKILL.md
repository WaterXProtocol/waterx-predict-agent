...
Skill version 2 · instructions version 4 · command schema version 1.
...
| `NEXT_IS_A_ROUTE_NOT_A_MANDATE` | `next` suggests reads. When it says stop, hand over to the person it names; when it asks for a value, ask the user. |
...
This document is a route, not the rules. The rules are the 35 in `AGENT_INSTRUCTIONS.md`, shipped beside this file. Where a build disagrees with it, `waterx-predict describe` wins: it reports what THIS installation can do.
