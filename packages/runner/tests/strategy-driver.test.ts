...
import { PredictAgentApiError, PredictAgentTransportError } from '@waterx/predict-agent-sdk';
...
const MARKET = 'mkt_btts_yes';
const NOW = later(T0, 60_000);
/** Sponsored bytes, as a value that would be a secret if it were real. */
const BYTES = 'c3BvbnNvcmVk';
...
const market = (overrides: Partial<PredictAgentMarket> = {}): PredictAgentMarket => ({
  marketId: MARKET,
...
const quote = (overrides: Partial<PredictQuote> = {}): PredictQuote => ({
  quoteId: 'q_1',
  marketId: MARKET,
  outcomeId: 'YES',
  side: 'SELL',
...
const created = (executionId: string): CreateExecutionResponseBody => ({
  executionId,
...
const apiError = (code = 'SLIPPAGE_EXCEEDED', httpStatus = 409): PredictAgentApiError =>
...
interface Recorder extends StrategyGateway {
...
const next = <T,>(answers: readonly (T | Error)[] | undefined, index: number, what: string): T => {
...
const gatewayOf = (script: Script = {}): Recorder => {
...
const signer: StrategySigner = { sign: async () => SIGNATURE };

const pricesAt = (...observed: (string | null)[]): PriceObserver => {
  let index = 0;
  return {
