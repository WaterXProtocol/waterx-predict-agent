...
Instructions version 4 · command schema version 1.
...
## Know which surface you are holding
...
### Three surfaces, one core — establish which one you hold

`IDENTIFY_YOUR_SURFACE_BEFORE_THE_FIRST_COMMAND`
...
### Name the surface you are missing, and stop there

`A_SURFACE_YOU_LACK_IS_NOT_A_CAPABILITY_TO_REBUILD`
...
### Loop on `runtime.next`, and obey its boundaries as well as its steps

`NEXT_IS_A_ROUTE_NOT_A_MANDATE`
...
### The execution policy belongs to the CLI; the delegation belongs to the API

`WHAT_GATES_A_WRITE_IS_THE_SURFACE_YOU_HOLD`
...
### Print the link, then wait for it — do not hand the wait to the person

`AN_AUTHORIZATION_LINK_IS_FOLLOWED_BY_A_POLL`
...
| Command | CLI | Tool | SDK | Kind | Confirmation | Summary |
| --- | --- | --- | --- | --- | --- | --- |
...
| `market.list` | `market list` | `waterx_predict_market_list` | `client.getMarkets()` | read | NOT_REQUIRED | List the tradeable market catalog. |
| `market.search` | `market search` | `waterx_predict_market_search` | `client.searchMarkets()` | read | NOT_REQUIRED | Resolve free text to one market id, or say it did not. |
| `market.get` | `market get` | `waterx_predict_market_get` | `client.getMarket()` | read | NOT_REQUIRED | Read one market by its server-resolved id. |
| `market.quote` | `market quote` | `waterx_predict_market_quote` | `client.getQuote()` | read | NOT_REQUIRED | Mint a short-lived executable quote. |
| `account.list` | `account list` | `waterx_predict_account_list` | `client.listAuthorizedAccounts()` | read | NOT_REQUIRED | The accounts this agent has been onboarded onto. |
| `account.status` | `account status` | `waterx_predict_account_status` | no — composed by the core | read | NOT_REQUIRED | One reading of capacity and exposure for an account. |
| `account.allowance` | `account allowance` | `waterx_predict_account_allowance` | `client.getAllowance()` | read | NOT_REQUIRED | Read the remaining API allowance and spendable balance. |
| `account.risk-limits` | `account risk-limits` | `waterx_predict_account_risk_limits` | `client.getEffectiveLimits()` | read | NOT_REQUIRED | Read the mandate this agent trades under, and what would refuse a write now. |
| `account.positions` | `account positions` | `waterx_predict_account_positions` | `client.getPositions()` | read | NOT_REQUIRED | List positions this agent opened, with cost basis and unrealized PnL. |
| `account.executions` | `account executions` | `waterx_predict_account_executions` | `client.listExecutions()` | read | NOT_REQUIRED | List this agent’s executions on one account. |
| `account.fills` | `account fills` | `waterx_predict_account_fills` | `client.getFills()` | read | NOT_REQUIRED | List confirmed fills, with the quote each was priced against. |
| `account.performance` | `account performance` | `waterx_predict_account_performance` | `client.getPerformance()` | read | NOT_REQUIRED | Read order outcomes, rejection reasons and realized PnL for this agent. |
| `order.preview` | `order preview` | `waterx_predict_order_preview` | no — composed by the core | read | NOT_REQUIRED | Resolve, price and policy-check an order without placing it. |
| `order.get` | `order get` | `waterx_predict_order_get` | `client.getExecution()` | read | NOT_REQUIRED | Read one execution by id. |
| `order.reconcile` | `order reconcile` | `waterx_predict_order_reconcile` | `client.waitForExecution()` | read | NOT_REQUIRED | Wait for one execution to reach a terminal state. |
| `order.execute` | `order execute` | `waterx_predict_order_execute` | `client.executeMarketOrder()` | write | REQUIRED_UNLESS_DELEGATED | Place one price-protected market order. |
| `order.execute-many` | `order execute-many` | `waterx_predict_order_execute_many` | `client.executeMany()` | write | REQUIRED_UNLESS_DELEGATED | Place several independent orders. |
| `strategy.create` | `strategy create` | `waterx_predict_strategy_create` | no — local Runner | write | REQUIRED_UNLESS_DELEGATED | Arm a durable conditional job on the local Runner. Can trade later, unattended. |
| `strategy.get` | `strategy get` | `waterx_predict_strategy_get` | no — local Runner | read | NOT_REQUIRED | Read one strategy: its state, its legs, and what is still unaccounted for. |
| `strategy.list` | `strategy list` | `waterx_predict_strategy_list` | no — local Runner | read | NOT_REQUIRED | List the strategies this Runner is holding. |
| `strategy.cancel` | `strategy cancel` | `waterx_predict_strategy_cancel` | no — local Runner | write | NOT_REQUIRED | Record a cancellation. Says whether it was APPLIED, which is not the same thing. |
| `strategy.events` | `strategy events` | `waterx_predict_strategy_events` | no — local Runner | read | NOT_REQUIRED | The transition and side-effect feed for one strategy, as of now. |
