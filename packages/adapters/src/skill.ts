...
export function renderAgentSkill(document: AgentSkillDocument = buildAgentSkill()): string {
  ...
  lines.push(
    `This document is a route, not the rules. The rules are the ${String(ruleCount())} in \`AGENT_INSTRUCTIONS.md\`, shipped beside this file, and the command inputs are in \`agent-commands.json\` in \`@waterx/predict-agent-schema\`. Where a build disagrees with either, \`waterx-predict describe\` wins: it reports what THIS installation can do.`,
    '',
  );
  return lines.join('\n');
}
