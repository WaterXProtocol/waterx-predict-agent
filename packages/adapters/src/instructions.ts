    ...
    rules: [
      {
        ...
        body: [
          ...
          'The CLI trades in DIRECT MODE by default (ADR-0013): public WaterX routes, the agent as the owner\'s on-chain delegate, no login and no Agent API. There the delegation is the whole grant and there is NO server-side risk profile — `account.risk-limits`, `account.allowance`, `account.executions`, `account.fills` and `account.performance` answer `CAPABILITY_UNAVAILABLE`, and `order.preview` reports `DIRECT_MODE_NO_SERVER_MANDATE`. That is the mode, not a missing grant: do not send the owner to write limits. The spending ceiling is the execution policy, and every transaction the backend builds is decoded and checked against the intent before it is signed; a mismatch is refused as `TRANSACTION_REFUSED` and nothing is signed. `runtime.describe` states which mode is running (`api.mode`).',
      ...
      },
      {
        ...
        body: [
          'The default policy is interactive — except on mainnet with no policy configured, where it is read-only until the operator sets `WATERX_PREDICT_POLICY=interactive` (ADR-0017); `next` says so, and enabling it is the operator\'s decision, never yours. Under interactive a write needs an approval that `order.preview` ISSUES for that exact normalized intent — that market, that side, that size, that bound. It expires after ten minutes and is spent by the one write it authorizes (ADR-0014): it does not authorise the next order, the same order twice, or the same order at a different size. A second order needs a second preview. It is not a credential.',
      ...
      },
      {
        ...
        body: [
          'Delegated-auto exists, is configured locally by the operator, and is bounded by the account owner\'s risk profile. Check what applies with `account.risk-limits` before promising a user that something will run unattended. In direct mode there is no risk profile to check: the delegated-auto scope (`maxBuyAmount`, `maxCumulativeBuyAmount`, `maxSellShares`, accounts, markets, `notAfter`) IS the ceiling, so quote it rather than a server limit.',
