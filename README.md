...
| [`packages/signer-keystore`](packages/signer-keystore) | The signer for **unattended** work, and the only one a Runner can use: `agent` unlocks an encrypted keystore once and holds the key; `sign` is spawned per request and holds nothing. Refuses a wrong token, a wrong address, and a directory other accounts can reach. | `delegated-auto` finally has a provider. The trade is real and stated: a decrypted key is resident, so load a **delegated agent wallet**, never the owner's |
