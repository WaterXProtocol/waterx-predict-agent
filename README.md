...
| [`packages/runner`](packages/runner) | The self-hosted local Runner. Today: the durable job store, the state machine, lease fencing, crash recovery, `UNKNOWN_PENDING` reconciliation against REST, `driveJob`, the scheduler that calls it on a tick, and a daemon that recovers, supervises leases and answers an authenticated local socket. | The loop exists; what it must be handed does not — **no price watcher and no signer are implemented**, so the shipped `runnerd` starts no scheduler and says so as `driving: false` |
