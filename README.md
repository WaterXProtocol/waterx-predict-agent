...
| [`packages/runner`](packages/runner) | The self-hosted local Runner. Today: the durable job store, the job state machine, lease fencing and crash recovery. | Durable half only — the **daemon, IPC and executor are not implemented**, so no job runs on its own |
