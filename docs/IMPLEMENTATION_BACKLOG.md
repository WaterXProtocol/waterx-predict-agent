...
boundary. What is still missing is the thing that *calls* it: no loop, no price
watcher behind `PriceObserver`, no signer behind `StrategySigner`. Inside the
daemon a Runner still claims a job, holds its lease, reports it and does nothing
with it. The daemon says so itself (`driving: false`, with the missing pieces
named), and any real agent command sent to its socket is refused. A strategy that
must outlive the invocation remains 2.6–2.8, and **no part of this build should
be read as a strategy running unattended**.
...
- A reserved package boundary is never evidence for the item that names it.
  ...
  is not a running strategy, however reachable the process holding it is. An
  exported `driveJob` does not change that: a function a caller must invoke is
  not a loop, and until the daemon runs one, `driving: false` is the true answer.
