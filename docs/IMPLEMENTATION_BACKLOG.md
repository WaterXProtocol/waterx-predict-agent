...
is a daemon now, and there is still **no executor and no conditional order** in
this build. A Runner claims a job, holds its lease and reports it — and then does
nothing with it: nothing quotes it, signs it, or asks it to reconcile. Resolving an
unresolved job is now implemented (`reconcileJob`), but it is a function a caller
invokes, not a loop the daemon runs. The daemon says so
itself (`driving: false`, with the missing pieces named), and any real agent
command sent to its socket is refused. A strategy that must outlive the invocation
remains 2.6–2.8, and no part of this build should be read as one working.
...
- A reserved package boundary is never evidence for the item that names it.
  ...
  is not a running strategy, however reachable the process holding it is.
