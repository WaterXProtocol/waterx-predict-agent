...
One of the two things the loop must be handed now exists as well:
...
whenever that feed cannot prove it is live. What is still missing is a signer
behind `StrategySigner`, and any local configuration from which `runnerd` could
build a driver — an observer over no stream is not a price source. So whether a
daemon drives
...
- A reserved package boundary is never evidence for the item that names it.
  ...
  `QuoteStreamPriceObserver` that exists does not change it either: an observer
  over a stream nothing opens is not a price source. The test of whether that has
  changed is `runnerd` printing `driving: true`, not the package being capable of
  it.
