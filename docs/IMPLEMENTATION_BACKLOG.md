...
`packages/runner` now holds the **durable half only** — the job store, the state
machine, lease fencing and crash recovery — and `packages/mcp` is still a reserved
boundary with **no implementation**. `packages/e2e` is a test harness that drives
the installed CLI; it ships to nobody, and a green harness suite is not a green
end-to-end.

What the CLI writes, it writes one order at a time from a one-shot process. There
is still no daemon, no executor and no conditional order in this build. The store
can now hold a job's state safely across a crash, but **nothing drives a job**:
no process claims one, quotes it, signs it or reconciles it, so a strategy that
must outlive the invocation remains 2.6–2.8 and no part of this build should be
read as one working.
...
| 2.6 | Runner daemon + local IPC | TODO | 2.5, 1.8. Unix domain socket; no Windows fallback in beta (ADR-0002). The store, the state machine and recovery are built and waiting for it; **nothing in this repo runs continuously**, so no job progresses on its own. Also missing above the store: the executor that drives a claimed job through the SDK, the heartbeat loop, and live `UNKNOWN_PENDING` reconciliation against REST. |
| 2.7 | Policy engine: `interactive` / `delegated-auto` / `read-only` | PARTIAL | ADR-0001 §9. Implemented and enforced for **one-shot CLI writes**: `packages/cli/src/policy.ts`, `packages/cli/tests/write-plane.test.ts` (34) + `signer.test.ts`. `--policy` may only narrow, never widen; `read-only` is enforced inside the signer; `delegated-auto` requires a file-only scope with mandatory per-order and per-run ceilings, an allow-listed account, side and market, and an unexpired `notAfter`, and every local check runs **before** any network read so an out-of-scope order costs nothing. The gap: no daemon-side policy and no policy evaluated across a durable job's lifetime (2.6) — a scope is checked at invocation, and there is nothing yet that could check it an hour later. The store now snapshots the policy that admitted a job (`JobPolicySnapshot`), so the value an hour-later check would read is durable; nothing performs that check. |
...
| 2.12 | Restart / kill / network-partition tests | PARTIAL | The **restart** half exists at the store layer: `packages/runner/tests/recovery.test.ts` (23) stages each crash point against a real database, closes it, reopens it and asserts what the next Runner does — re-arm on the same key, `UNKNOWN_PENDING` on an unresolved attempt, reconcile on a recorded execution id, and hands off rather than racing when another instance holds the lease. These are in-process reopen tests, not a killed process: no `SIGKILL`, no partition, and no server on the other end. Kill and network-partition need something that runs (2.6). |
...
- A reserved package boundary is never evidence for the item that names it.
  `packages/mcp` existing does not advance 3.2, and `packages/runner` holding a
  durable store does not advance 2.6 — a job that nothing drives is not a
  running strategy.
