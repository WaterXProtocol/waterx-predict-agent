...
What is still missing is what the loop must be handed to run: no price watcher
behind `PriceObserver`, no signer behind `StrategySigner`, and no local
configuration from which `runnerd` could build either. So whether a daemon drives
...
- A reserved package boundary is never evidence for the item that names it.
  ...
  test of whether that has changed is `runnerd` printing `driving: true`, not the
  package being capable of it.
